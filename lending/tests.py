...
class TestLoanService:
    ...
    def test_create_loan_rounding_drift(self, users_and_accounts):
        ...
        # This combination yields a rounding drift
        ...
        # Expected total interest = (1000 * 5.0 * 7) / 1200 = 29.166666666666667
        # Total repayable quantized to 2 decimal places = 1000 + 29.17 = 1029.17
        # Monthly amount = (1029.17 / 7).quantize(Decimal("0.01")) = 147.02
        # First 6 installments: 147.02
        # Last installment: 1029.17 - (6 * 147.02) = 1029.17 - 882.12 = 147.05
        installments = list(Installment.objects.filter(loan=loan).order_by("due_date"))
        assert len(installments) == 7

        for inst in installments[:-1]:
            assert inst.amount == Decimal("147.02")

        assert installments[-1].amount == Decimal("147.05")
