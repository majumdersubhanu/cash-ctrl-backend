...
def users_and_accounts():
    borrower = UserService.create_user(email="borrower@test.com", password="pwd", first_name="Bob")
    lender = UserService.create_user(email="lender@test.com", password="pwd", first_name="Lenny")
    
    # We create basic accounts for testing payment flows
    borrower_acc = WalletAccount.objects.create(user=borrower, name="Borrower Wallet", balance=Decimal("1500.00"), currency="USD", wallet_provider="Test")
    lender_acc = WalletAccount.objects.create(user=lender, name="Lender Wallet", balance=Decimal("5000.00"), currency="USD", wallet_provider="Test")
