...
class LoanService:
    ...
    def create_loan(borrower, amount, interest_rate, duration_months, lender=None):
        ...
        total_interest = (amount * interest_rate * Decimal(duration_months)) / (Decimal('100') * Decimal('12'))
    ...
    def pay_installment(installment, account):
        ...
        if not installment.loan.installments.filter(status__in=['PENDING', 'OVERDUE']).exists():
