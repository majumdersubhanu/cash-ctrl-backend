...
class OnboardingLendingIntegrationTest(APITestCase):
    ...
    def test_onboarding_to_loan_flow(self):
        ...
        doc_file = SimpleUploadedFile(
            "id.jpg", b"fake_content", content_type="image/jpeg"
        )
        KYCDocument.objects.create(
            profile=profile, document_type="ID_CARD", document_file=doc_file
        )
        ...
        # Verify Audit Log
        self.assertTrue(
            AuditLog.objects.filter(user=self.user, action="KYC_SUBMITTED").exists()
        )
