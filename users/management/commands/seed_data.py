...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write("Starting high-fidelity Mass Data Seeding (5K Users / 500K Transactions)...")
        ...
        if SplitGroup.objects.count() < 500:
            ...
            for i in range(500):
                ...
                groups.append(SplitGroup(name=fake.company() + " Trip", creator=creator))
        ...
        for group in all_groups:
            ...
            for _ in range(5):
                ...
                SplitExpense.objects.create(
                    group=group, description=fake.catch_phrase(),
                    paid_by=payer, amount=amount, currency='USD'
                )
        ...
        for i in range(500):
            ...
            goals.append(SavingsGoal(
                user=random.choice(all_users), name=fake.bs(),
                target_amount=target, current_amount=target * Decimal(random.uniform(0.1, 0.9)),
                target_date=timezone.now().date() + timedelta(days=random.randint(30, 365))
            ))
        ...
        for i in range(5000):
            notifications.append(Notification(
                user=random.choice(all_users), title=fake.sentence(nb_words=4),
                message=fake.text(), type=random.choice(['ALERT', 'WARNING', 'INFO'])
            ))
        Notification.objects.bulk_create(notifications, batch_size=1000, ignore_conflicts=True)

        self.stdout.write(self.style.SUCCESS(f"MASS DATA SEEDING COMPLETE! The DB has {User.objects.count()} Users and {Transaction.objects.count()} Transactions."))
