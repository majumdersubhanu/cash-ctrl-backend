...
def p2p_network_analytics(request):
    ...
    top_lenders = (
        User.objects.annotate(
            loans_count=Count("loans_given"), total_lent=Sum("loans_given__amount")
        )
        .filter(loans_count__gt=0)
        .order_by("-total_lent")[:10]
    )

    # top borrowers
    top_borrowers = (
        User.objects.annotate(
            loans_count=Count("loans_taken"), total_borrowed=Sum("loans_taken__amount")
        )
        .filter(loans_count__gt=0)
    ...
    groups = SplitGroup.objects.annotate(
        member_count=Count("members"), expense_count=Count("expenses")
